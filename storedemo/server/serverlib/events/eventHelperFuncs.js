...
const redisCrossCheck = async (set, redis, next, responseDb) => {
  for(const ele of set) {
    try {
      ...
      if((!success || success === undefined) && responseDb[ele]) {
        console.log('there is a client that used to be connected to this server');
        const failure = await redis.smembers('SC' + ele)
        for(const docs of failure){
          await redis.srem('SD' + docs, ele)
        }
      }
    }catch (err) {
...
eventRouteHelperFuncs.monitorListingsUsingEventEmitter =  async (client, redis, responseDb, timeInMs = 6000000, pipeline = []) => {
  ...
  changeStream.on('change', async (next) => {
    ...
    if(next.operationType === 'insert'){
      ...
      console.log('changeStreamDocId', changeStreamDocId)
      console.log('redisSubDocKey', redisSubDocKey)
